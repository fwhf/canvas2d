<template>
  <div class="drawingBoardFlow">
    ...
    <div class="config">
      ...
      <div class="configItem" v-if="config.canvas">
      ...
      <div class="configItem" v-if="config.graph">
      ...
      <div class="configItem" v-if="config.line">
    ...
    <div class="rightMenu" v-if="rightMenu && rightMenu.show" :style="rightMenu.style">
    ...
    <div class="canvasBtn">
      ...
      <button class="reset" @click="initConfig">重置配置</button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    initGraphGroup() {
      let index = 0
      this.graphGroup.children.forEach(item => {
        item.children.forEach(graph => {
          if (graph.GraphClass) {
            new graph.GraphClass({
              canvas: this.$refs.graphItemRef[index]
            })
          }
          index++
        })
    ...
    // 配置初始化
    initConfig() {
      this.config.canvas = JSON.parse(JSON.stringify(this.originConfig.canvas))
      this.config.graph = JSON.parse(JSON.stringify(this.originConfig.graph))
      this.config.line = JSON.parse(JSON.stringify(this.originConfig.line))
      this.canvasChange()
    },
    // 配置 画板修改
    canvasChange() {
      if (!this.DrawingBoardFlowClass) {
        return
      }
      this.DrawingBoardFlowClass.saveImagePadding = this.config.canvas.saveImagePadding
      this.DrawingBoardFlowClass.bgColor = this.config.canvas.bgColor
      this.DrawingBoardFlowClass.gridStyle = this.config.canvas.gridStyle
      this.DrawingBoardFlowClass.reDraw()
    },
    // 配置 图形、线段修改
    graphChange(type) {
      if (this.DrawingBoardFlowClass.focusGraphOrLine) {
        if (this.DrawingBoardFlowClass.focusGraphOrLine.graphType === 'graph') {
          this.DrawingBoardFlowClass.focusGraphOrLine.style = objectMerge(this.DrawingBoardFlowClass.focusGraphOrLine.style, this.config.graph.graphStyle)
          if (type.indexOf('text') > -1) {
            this.DrawingBoardFlowClass.focusGraphOrLine.formatTextXY()
          }
          this.DrawingBoardFlowClass.reDraw()
        } else if (this.DrawingBoardFlowClass.focusGraphOrLine.graphType === 'line') {
          this.DrawingBoardFlowClass.focusGraphOrLine.style = objectMerge(this.DrawingBoardFlowClass.focusGraphOrLine.style, this.config.line.lineStyle)
          this.DrawingBoardFlowClass.reDraw()
        }
      }
    },
        ...
        list: [
          {
            ...
            show: true,
            cb: () => {
              this.DrawingBoardFlowClass.resetGraphListIndex()
          ...
          },
          {
            ...
            show: true,
            cb: () => {
              this.DrawingBoardFlowClass.delGraphToList()
              this.rightMenu.show = false
            }
          },
          {
            ...
            show: true,
            cb: () => {
              this.rightMenu.ctrlC = this.DrawingBoardFlowClass.focusGraphOrLine
              this.rightMenu.show = false
            }
          },
          {
            ...
            show: true,
            cb: () => {
              const canvasDom = this.$refs.drawingBoardFlowCanvas
              let data = {
                canvas: canvasDom,
                scaleC: this.rightMenu.ctrlC.scaleC
              }
              let x = (this.rightMenu.xy.x - canvasDom.getBoundingClientRect().left) * this.DrawingBoardFlowClass.dpr
              let y = (this.rightMenu.xy.y - canvasDom.getBoundingClientRect().top) * this.DrawingBoardFlowClass.dpr
              x = x - ((x - this.DrawingBoardFlowClass.canvas.width / 2) % (this.config.canvas.gridStyle.width * this.DrawingBoardFlowClass.scaleC))
              y = y - ((y - this.DrawingBoardFlowClass.canvas.height / 2) % (this.config.canvas.gridStyle.width * this.DrawingBoardFlowClass.scaleC))
              if (this.rightMenu.ctrlC.graphType === 'graph') {
                data = objectMerge(data, {
                  xy: {
                    x:
                      this.DrawingBoardFlowClass.canvas.width / 2 +
                      (x - this.DrawingBoardFlowClass.canvas.width / 2) / this.DrawingBoardFlowClass.scaleC +
                      this.rightMenu.ctrlC.style.width / 2,
                    y:
                      this.DrawingBoardFlowClass.canvas.height / 2 +
                      (y - this.DrawingBoardFlowClass.canvas.height / 2) / this.DrawingBoardFlowClass.scaleC +
                      this.rightMenu.ctrlC.style.height / 2
                  },
                  style: this.rightMenu.ctrlC.style,
                  content: this.rightMenu.ctrlC.content,
                  drawContent: this.rightMenu.ctrlC.drawContent,
                  autoDraw: false
                })
                let newGraph = null
                if (this.rightMenu.ctrlC.asName === 'TextGraphAutoWH') {
                  newGraph = new TextGraph(data)
                } else {
                  newGraph = new this.rightMenu.ctrlC.constructor(data)
                }
                newGraph.formatTextXY()
                this.DrawingBoardFlowClass.addGraphToList(newGraph)
              }
              if (this.rightMenu.ctrlC.graphType === 'line') {
                const vertex = []
                const translate = {
                  x:
                    this.DrawingBoardFlowClass.canvas.width / 2 +
                    (x - this.DrawingBoardFlowClass.canvas.width / 2) / this.DrawingBoardFlowClass.scaleC -
                    this.rightMenu.ctrlC.vertex[0].x,
                  y:
                    this.DrawingBoardFlowClass.canvas.height / 2 +
                    (y - this.DrawingBoardFlowClass.canvas.height / 2) / this.DrawingBoardFlowClass.scaleC -
                    this.rightMenu.ctrlC.vertex[0].y
                }
                this.rightMenu.ctrlC.vertex.forEach(item => {
                  vertex.push({
                ...
                const newLine = new this.rightMenu.ctrlC.constructor(data)
                if (this.rightMenu.ctrlC.contentArr.length) {
                  this.rightMenu.ctrlC.contentArr.forEach(item => {
                ...
                this.DrawingBoardFlowClass.addGraphToList(newLine)
                if (newLine.contentArr.length) {
                  newLine.contentArr.forEach(item => {
                    this.DrawingBoardFlowClass.addGraphToList(item)
                  })
                }
              }
              this.rightMenu.show = false
          ...
          },
          {
            ...
            show: false,
            cb: () => {
              this.DrawingBoardFlowClass.focusGraphOrLine.delVertex(this.rightMenu.rightSelectionIndex, this.DrawingBoardFlowClass)
              this.rightMenu.show = false
              this.DrawingBoardFlowClass.vertexFocusRightSelectionIndex = null
              this.DrawingBoardFlowClass.reDraw()
            }
          }
        ]
    ...
    initFlowCanvas() {
      const drawingBoardFlowObj = {
        canvas: this.$refs.drawingBoardFlowCanvas,
        eventFlag: true,
        bgColor: this.config.canvas.bgColor,
        saveImagePadding: this.config.canvas.saveImagePadding,
        gridStyle: this.config.canvas.gridStyle,
        mousemoveHoverFlag: true,
        mouseupBtn0Cb: this.mouseupBtn0Cb,
        mouseupBtn2Cb: this.mouseupBtn2Cb
      }
      ...
      if (flowData) {
        ...
        this.config.canvas.gridStyle = flowData.gridStyle
        drawingBoardFlowObj.bgColor = flowData.bgColor
        this.config.canvas.bgColor = flowData.bgColor
        drawingBoardFlowObj.saveImagePadding = flowData.saveImagePadding * dprIndex
        this.config.canvas.saveImagePadding = flowData.saveImagePadding * dprIndex

        drawingBoardFlowObj.scaleCIndex = flowData.scaleCIndex
        this.DrawingBoardFlowClass = new DrawingBoardFlow(drawingBoardFlowObj)

        const translate = {
          x: this.DrawingBoardFlowClass.centerXY.x - flowData.centerXY.x,
          y: this.DrawingBoardFlowClass.centerXY.y - flowData.centerXY.y
        }
        if (flowData.graphList.length) {
          ...
          flowData.graphList.forEach(item => {
            ...
            if (NewClass) {
              const data = {
                canvas: this.$refs.drawingBoardFlowCanvas,
                scaleC: this.DrawingBoardFlowClass.scaleC,
                autoDraw: false,
                style: item.style,
                id: item.id,
                content: item.content
              ...
              this.DrawingBoardFlowClass.addGraphToList(new NewClass(data))
            }
          })
          this.DrawingBoardFlowClass.graphList.forEach(item => {
            if (item.graphType === 'graph' && item.lineArr.length) {
              for (let i = 0; i < item.lineArr.length; i++) {
                item.lineArr[i].line = this.DrawingBoardFlowClass.graphList.filter(line => {
                  return line.id === item.lineArr[i].id
                })[0]
              }
            } else if (item.graphType === 'line' && item.contentArr.length) {
              for (let i = 0; i < item.contentArr.length; i++) {
                item.contentArr[i] = this.DrawingBoardFlowClass.graphList.filter(graph => {
    ...
    mouseupBtn0Cb() {
      ...
      if (this.DrawingBoardFlowClass.focusGraphOrLine) {
        if (this.DrawingBoardFlowClass.focusGraphOrLine.graphType === 'graph') {
          Object.keys(this.config.graph.graphStyle).forEach(attr => {
            this.config.graph.graphStyle[attr] = this.DrawingBoardFlowClass.focusGraphOrLine.style[attr]
          })
        } else if (this.DrawingBoardFlowClass.focusGraphOrLine.graphType === 'line') {
          Object.keys(this.config.line.lineStyle).forEach(attr => {
            this.config.line.lineStyle[attr] = this.DrawingBoardFlowClass.focusGraphOrLine.style[attr]
          })
        }
      }
    },
    // 右击回调
    mouseupBtn2Cb(xy, vertexFocusRightSelectionIndex) {
      this.rightMenu.list.forEach(item => {
        item.show = false
      })
      let length = 0
      if (this.DrawingBoardFlowClass.focusGraphOrLine) {
        this.rightMenu.list[0].show = true
        this.rightMenu.list[1].show = true
        this.rightMenu.list[2].show = true
        length = 3
      }
      if (this.rightMenu.ctrlC) {
        this.rightMenu.list[3].show = true
        length += 1
      }
      if (vertexFocusRightSelectionIndex) {
        this.rightMenu.rightSelectionIndex = vertexFocusRightSelectionIndex
        this.rightMenu.list[4].show = true
        length += 1
      }
      if (length) {
        ...
        let left = xy.x + this.rightMenu.translate.x
        let top = xy.y + this.rightMenu.translate.y
        if (left + width > bodyClientWidth) {
          left = left - width - this.rightMenu.translate.x * 2
        }
        if (top + height > bodyClientHeight) {
          top = top - height - this.rightMenu.translate.y * 2
        }
        left += 'px'
        top += 'px'
        this.rightMenu.style = {
          left,
          top
        }
      } else {
    ...
    // 保存数据
    saveData() {
      const saveData = this.DrawingBoardFlowClass.save()
    ...
    dowload() {
      const saveData = this.DrawingBoardFlowClass.save()
      this.downloadImage(saveData.imageData.base64, '流程图')
    },
    downloadImage(base64, downloadName) {
    ...
    // 图形拖放逻辑
    fictitiousGridDomInit(x, y) {
      this.fictitiousGridAttr.width = this.originConfig.graph.graphStyle.width
      this.$refs.fictitiousGrid.style.width = (this.fictitiousGridAttr.width / this.DrawingBoardFlowClass.dpr) * this.DrawingBoardFlowClass.scaleC + 'px'
      this.fictitiousGridAttr.height = this.CurMoveGraphClass.benchmarkType
        ? this.originConfig.graph.graphStyle.height
        : this.originConfig.graph.graphStyle.height * 2
      this.$refs.fictitiousGrid.style.height = (this.fictitiousGridAttr.height / this.DrawingBoardFlowClass.dpr) * this.DrawingBoardFlowClass.scaleC + 'px'
      this.fictitiousGridMove(x, y)
      this.fictitiousGridAttr.show = true
    },
    fictitiousGridMove(x, y) {
      this.$refs.fictitiousGrid.style.left = x + 'px'
      this.$refs.fictitiousGrid.style.top = y + 'px'
    },
    graphDown(graphItem) {
      const e = window.event
      this.CurMoveGraphClass = graphItem.GraphClass
      document.addEventListener('mousemove', this.graphMove)
      document.addEventListener('mouseup', this.graphUp)

      this.fictitiousGridDomInit(e.clientX, e.clientY)
      e.preventDefault()
    },
    graphMove(e) {
      if (!this.CurMoveGraphClass) {
        return
      }
      this.fictitiousGridMove(e.clientX, e.clientY)
    },
    graphUp(e) {
      const canvasDom = this.$refs.drawingBoardFlowCanvas
      if (
        ...
        let x = (e.clientX - canvasDom.getBoundingClientRect().left) * this.DrawingBoardFlowClass.dpr
        let y = (e.clientY - canvasDom.getBoundingClientRect().top) * this.DrawingBoardFlowClass.dpr
        x = x - ((x - this.DrawingBoardFlowClass.canvas.width / 2) % (this.config.canvas.gridStyle.width * this.DrawingBoardFlowClass.scaleC))
        y = y - ((y - this.DrawingBoardFlowClass.canvas.height / 2) % (this.config.canvas.gridStyle.width * this.DrawingBoardFlowClass.scaleC))
        if (this.CurMoveGraphClass.graphType === 'graph') {
          data = objectMerge(data, {
            xy: {
              x:
                this.DrawingBoardFlowClass.canvas.width / 2 +
                (x - this.DrawingBoardFlowClass.canvas.width / 2) / this.DrawingBoardFlowClass.scaleC +
                this.fictitiousGridAttr.width / 2,
              y:
                this.DrawingBoardFlowClass.canvas.height / 2 +
                (y - this.DrawingBoardFlowClass.canvas.height / 2) / this.DrawingBoardFlowClass.scaleC +
                this.fictitiousGridAttr.height / 2
            },
            style: objectMerge(this.config.graph.graphStyle, {
              width: this.fictitiousGridAttr.width,
              height: this.fictitiousGridAttr.height
            })
          })
          this.DrawingBoardFlowClass.addGraphToList(new this.CurMoveGraphClass(data))
        } else if (this.CurMoveGraphClass.graphType === 'line') {
          let vertex = []
          if (this.CurMoveGraphClass.asName === 'CurveLine') {
            vertex = [
              {
                x: this.DrawingBoardFlowClass.canvas.width / 2 + (x - this.DrawingBoardFlowClass.canvas.width / 2) / this.DrawingBoardFlowClass.scaleC,
                y: this.DrawingBoardFlowClass.canvas.height / 2 + (y - this.DrawingBoardFlowClass.canvas.height / 2) / this.DrawingBoardFlowClass.scaleC
              },
              {
                x:
                  this.DrawingBoardFlowClass.canvas.width / 2 +
                ...
                y:
                  this.DrawingBoardFlowClass.canvas.height / 2 +
                  (y - this.DrawingBoardFlowClass.canvas.height / 2) / this.DrawingBoardFlowClass.scaleC +
                  this.fictitiousGridAttr.height
              },
              {
                x:
                  this.DrawingBoardFlowClass.canvas.width / 2 +
                  (x - this.DrawingBoardFlowClass.canvas.width / 2) / this.DrawingBoardFlowClass.scaleC +
                  this.fictitiousGridAttr.width,
                y: this.DrawingBoardFlowClass.canvas.height / 2 + (y - this.DrawingBoardFlowClass.canvas.height / 2) / this.DrawingBoardFlowClass.scaleC
              }
            ]
          } else {
          ...
          data = objectMerge(data, {
            vertex,
            style: objectMerge(this.config.line.lineStyle, {
              width: this.fictitiousGridAttr.width,
              height: this.fictitiousGridAttr.height
            })
          })
      ...
      document.removeEventListener('mousemove', this.graphMove)
      document.removeEventListener('mouseup', this.graphUp)
      this.fictitiousGridAttr.show = false
      this.CurMoveGraphClass = null
    }
  },
  beforeDestroy() {
    document.removeEventListener('mousemove', this.graphMove)
    document.removeEventListener('mouseup', this.graphUp)
    this.DrawingBoardFlowClass && this.DrawingBoardFlowClass.removeEvent && this.DrawingBoardFlowClass.removeEvent()
  }
}
</script>
